        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-primary flex items-center gap-2">
              <Users /> {t('Community Presets')}
            </h1>
            <p className="text-sm text-text-secondary">{t('library.discoverCommunityPresets')}</p>
          </div>
        </div>
      </header>
      ...
      <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
        <div className="relative">
          <Input
            ...
            placeholder={t('library.searchPresetsPlaceholder')}
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <div className="relative">
            ...
            >
              <option value="name">{t('library.nameAz')}</option>
            </select>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 -mr-2">
        {isLoading ? (
          <div className="flex items-center justify-center h-full text-text-secondary">
            ...
            {t('Fetching presets from GitHub...')}
          </div>
        ) : (
          <motion.div
                  ...
                  >
                    <div className="relative w-full aspect-square bg-bg-primary flex items-center justify-center text-text-secondary">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <Button
                          ...
                          className="shadow-lg"
                        >
                          {status === 'idle' && <>{t('common.actions.save')}</>}
                          {status === 'downloading' && <><Loader2 size={14} className="mr-2 animate-spin" /> {t('Saving...')}</>}
                          {status === 'success' && <><CheckCircle2 size={14} className="mr-2" /> {t('Saved')}</>}
                        </Button>
                      </div>
                    </div>
                    <div className="p-3 text-center">
                      ...
                      <p className="text-xs text-text-secondary font-['cursive'] italic mt-1">
                        {t('library.byCreator', { creator: preset.creator })}
                      </p>
                    </div>
                  </motion.div>
                );
        ...
        {allPreviewsLoaded && (
          ...
          >
            <p>{t('library.wantPresetFeatured')}</p>
            <a
